...
def read_files(train_label_file, test_label_file):
    ...
    save_variable(tmp_dict, "/mnt/efs-tcga/HEAL_Workspace/outputs/parameter.conf")
    ...
    if not MLMC:
        ...
        save_dir = "/mnt/efs-tcga/HEAL_Workspace/figures/"
        ...
        # Salva a figura
        plt.savefig(os.path.join(save_dir, "patient_vs_class_distribution.png"), dpi=400)
        plt.clf()
        plt.cla()
        plt.close()
...
def show_tiles_distribution(df):
    ...
    plt.grid(alpha=0.5, axis='y')
    plt.savefig("/mnt/efs-tcga/HEAL_Workspace/figures/tile_vs_class_distribution.png", dpi = 400)
    plt.clf()
    plt.cla()
    plt.close()
...
def split_data(train_df, test_df, test_ratio, MLMC):
    ...
    if test_df == None:
        ...
        test_tmp_df.to_csv("/mnt/efs-tcga/HEAL_Workspace/outputs/test_label_file_tiled.csv",
...
def write_train_file(train_df, train_indices, val_indices):
    ...
    for i in range(10):
        ...
        pool.join()
        global train_img_label_df
        train_img_label_df.to_csv("/mnt/efs-tcga/HEAL_Workspace/outputs/train_fold_%d.csv" % i, encoding='utf-8', index=False)
        ...
        pool.join()
        global val_img_label_df
        val_img_label_df.to_csv("/mnt/efs-tcga/HEAL_Workspace/outputs/val_fold_%d.csv" % i, encoding='utf-8', index=False)
